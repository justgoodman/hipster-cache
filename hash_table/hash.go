...
// where aplha=n/m is the load factor of the hash table
func (c *ComplexStringHash) CalculateHash(value string) int64 {
...
func getRandom(maxValue int64) int64 {
	return rand.Int63n(maxValue-2) + 1
}
...
func NewIntegerHash(p, m int64) *IntegerHash {
...
func (s *StringHash) CalculateHash(value string) int64 {
	var result int64
	var coef int64
	coef = 1
	for _, character := range value {
		result += int64(character) * coef % s.p
		coef *= s.x
	}
	return result % s.p
}
...
func (i *IntegerHash) CalculateHash(value int64) int64 {
