...
func (l *SetDictOperation) SetValue(sourceValue *interface{}, value interface{}) (valueBytesSize int) {
	...
	case *Chain:
		...
		if element != nil {
			lastBytesSize := element.bytesSize
			element.updateValue(stringValue)
			chain.bytesSize += element.bytesSize - lastBytesSize
			valueBytesSize = chain.bytesSize
			break
		}
	...
	case string:
		...
		*sourceValue = interface{}(newChain)
		...
		valueBytesSize = newChain.bytesSize
	default:
